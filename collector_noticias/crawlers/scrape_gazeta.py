#!/usr/bin/env python3
"""
Gazeta do Povo News Scraper
============================

Scrapes news articles from Gazeta do Povo /ultimas-noticias/ section.
Uses binary search to find max pages, then scrapes all articles.
Saves to ../scraped/gazeta/ directory.

Usage:
    python3 scrape_gazeta.py [--max-pages N] [--start-page N] [--no-auto-detect]
"""
...
OUTPUT_DIR = Path(__file__).parent.parent / "scraped" / "gazeta"
...
def scrape_article(url: str, session: requests.Session) -> dict:
    """
    Scrape a single article from Gazeta do Povo
    ...
    try:
        ...
        # Extract date from URL pattern (YYYY/MM/DD) or time tag
...
def scrape_page(page_num: int, session: requests.Session) -> list:
    ...
    """
    # Page 1 uses /ultimas-noticias/ (no number), pages 2+ use /2/, /3/, etc.
    if page_num == 1:
    ...
    try:
        ...
        # Find article cards specifically (not all links with title)
        article_cards = soup.find_all('article', class_=lambda x: x and 'cardDefault_card-container' in x)
        article_links = []
        
        for article in article_cards:
            # Find the main link inside each article card
            ...
            if link:
                ...
                if href.startswith('/'):
                    href = f'https://www.gazetadopovo.com.br{href}'
                
                # Remove URL fragments and query params for deduplication
...
def save_article(article: dict, index: int):
    ...
    filename = OUTPUT_DIR / f"gazeta_{index:04d}.json"
...
def main(max_pages: int = None, start_page: int = 1, auto_detect: bool = True):
    ...
    """
    print("\n" + "="*80)
    print("GAZETA DO POVO NEWS SCRAPER")
    print("="*80 + "\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Gazeta do Povo news")
    parser.add_argument(
        '--max-pages',
        type=int,
        default=None,
