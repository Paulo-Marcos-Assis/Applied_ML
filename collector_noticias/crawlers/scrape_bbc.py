#!/usr/bin/env python3
"""
BBC Portuguese News Scraper
============================

Scrapes all available news articles from BBC Portuguese.
Saves to scraped/bbcportuguese/ directory.

Usage:
    python3 scrape_bbc.py [--max-pages N] [--start-page N]
"""
...
# Configuration
BASE_URL = "https://www.bbc.com/portuguese/topics/cz74k717pw5t?page={}"
OUTPUT_DIR = Path(__file__).parent.parent / "scraped" / "bbcportuguese"
...
def scrape_article(url: str, session: requests.Session) -> dict:
    """
    Scrape a single article from BBC Portuguese
...
def scrape_page(page_num: int, session: requests.Session) -> list:
    ...
    try:
        ...
        article_links = [
            ...
            if '/portuguese/articles/' in a['href']
        ]
        
        # Remove duplicates and make full URLs
        article_links = list(set(article_links))
        full_urls = [
            link if link.startswith('http') else f'https://www.bbc.com{link}'
...
def find_max_page(session: requests.Session) -> int:
    ...
    This is the same algorithm used in the main collector.
...
def save_article(article: dict, index: int):
    ...
    filename = OUTPUT_DIR / f"bbcportuguese_{index:04d}.json"
...
def main(max_pages: int = None, start_page: int = 1, auto_detect: bool = True):
    ...
    """
    print("\n" + "="*80)
    print("BBC PORTUGUESE NEWS SCRAPER")
    print("="*80 + "\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape BBC Portuguese news")
    parser.add_argument(
        '--max-pages',
        type=int,
        default=None,
