#!/usr/bin/env python3
"""
Script para treinar SVM com diferentes vetorizacoes
...
def train_and_evaluate_svm(X_train, y_train, X_dev, y_dev, vec_name):
    ...
    print(f"\n{'='*70}")
    print(f"TREINANDO SVM COM {vec_name}")
    print(f"{'='*70}")
    
    # SVM com kernel linear
    model = SVC(kernel='linear', probability=True, random_state=42)
    print("Configuracao: SVC(kernel='linear', probability=True)")
...
def compare_vectorizations(results, output_dir):
    ...
    print("\n" + "="*70)
    print("COMPARACAO ENTRE VETORIZACOES (SVM)")
    print("="*70)
    ...
    csv_path = os.path.join(output_dir, f'svm_vectorizations_comparison_{timestamp}.csv')
...
def plot_comparison(df_comparison, output_dir):
    ...
    fig.suptitle('SVM: Vectorization Comparison', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, f'svm_vectorizations_comparison_{timestamp}.png')
...
def save_results(results, output_dir):
    ...
    for vec_name, vec_results in results.items():
        model_path = os.path.join(output_dir, f'svm_{vec_name.lower().replace(" ", "_").replace("-", "_")}_{timestamp}.pkl')
    ...
    metadata_path = os.path.join(output_dir, f'svm_all_vectorizations_metadata_{timestamp}.pkl')
...
def main():
    ...
    OUTPUT_DIR = os.path.join(BASE_DIR, "training/results/svm_vectorizations")
    ...
    print("="*70)
    print("SVM COM MULTIPLAS VETORIZACOES")
    print("="*70)
    ...
    try:
        ...
        for vec_name, vec_config in vectorizations.items():
            ...
            # Treinar e avaliar
            model, cv_results, dev_metrics = train_and_evaluate_svm(
